...
const TypeAuditManagementScreen = () => {
  ...
  const handleDelete = (id) => {
    Alert.alert(
      'Supprimer',
      'Êtes-vous sûr de vouloir supprimer ce type d\'audit ?',
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              await api.delete(getApiUrl(`${API_PATHS.TYPES_AUDIT}${id}/`));
              fetchData();
            } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing) {
        await api.put(getApiUrl(`${API_PATHS.TYPES_AUDIT}${currentId}/`), formData);
      } else {
        await api.post(getApiUrl(API_PATHS.TYPES_AUDIT), formData);
      }
  ...
  const renderHeader = () => (
    <View style={styles.tableHeader}>
      ...
      <View style={[styles.headerCell, { flex: 1.5 }]}><Text style={styles.headerText}>Type d'Audit</Text></View>
      <View style={[styles.headerCell, { flex: 1 }]}><Text style={styles.headerText}>Sections</Text></View>
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <Text style={styles.title}>Types d'Audit</Text>
        </View>
...
export default TypeAuditManagementScreen;
