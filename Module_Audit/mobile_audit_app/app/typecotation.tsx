...
const TypeCotationManagementScreen = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get(getApiUrl(API_PATHS.TYPE_COTATION));
  ...
  const handleDelete = (id) => {
    Alert.alert(
      'Supprimer',
      'Êtes-vous sûr de vouloir supprimer ce type de cotation ?',
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              await api.delete(getApiUrl(`${API_PATHS.TYPE_COTATION}${id}/`));
              fetchData();
            } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing) {
        await api.put(getApiUrl(`${API_PATHS.TYPE_COTATION}${currentId}/`), formData);
      } else {
        await api.post(getApiUrl(API_PATHS.TYPE_COTATION), formData);
      }
  ...
  const renderHeader = () => (
    <View style={styles.tableHeader}>
      ...
      <View style={[styles.headerCell, { flex: 1 }]}><Text style={styles.headerText}>Type de Cotation</Text></View>
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <Text style={styles.title}>Types de Cotation</Text>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <Text style={styles.inputLabel}>Nom du type de cotation</Text>
              <TextInput
...
export default TypeCotationManagementScreen;
