...
const ProcessusManagementScreen = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get(getApiUrl(API_PATHS.PROCESSUS));
  ...
  const handleDelete = (id) => {
    Alert.alert(
      'Supprimer',
      'Êtes-vous sûr de vouloir supprimer ce processus ?',
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              await api.delete(getApiUrl(`${API_PATHS.PROCESSUS}${id}/`));
              fetchData();
            } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing) {
        await api.put(getApiUrl(`${API_PATHS.PROCESSUS}${currentId}/`), formData);
      } else {
        await api.post(getApiUrl(API_PATHS.PROCESSUS), formData);
      }
  ...
  const renderHeader = () => (
    <View style={styles.tableHeader}>
      ...
      <View style={[styles.headerCell, { flex: 1 }]}><Text style={styles.headerText}>Nom du Processus</Text></View>
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <Text style={styles.title}>Processus</Text>
        </View>
...
export default ProcessusManagementScreen;
