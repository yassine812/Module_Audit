...
const SectionsManagementScreen = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get(getApiUrl(API_PATHS.SECTIONS));
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              await api.delete(getApiUrl(`${API_PATHS.SECTIONS}${id}/`));
              fetchData();
            } catch (error: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing) {
        await api.put(getApiUrl(`${API_PATHS.SECTIONS}${currentId}/`), formData);
      } else {
        await api.post(getApiUrl(API_PATHS.SECTIONS), formData);
      }
  ...
  const renderHeader = () => (
    <View style={styles.tableHeader}>
      ...
      <View style={[styles.headerCell, { flex: 1 }]}><Text style={styles.headerText}>Nom de la Section</Text></View>
...
export default SectionsManagementScreen;
