...
const TypePreuveManagementScreen = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get(getApiUrl(API_PATHS.TYPE_PREUVE));
  ...
  const handleDelete = (id) => {
    Alert.alert(
      'Supprimer',
      'Êtes-vous sûr de vouloir supprimer ce type de preuve ?',
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              await api.delete(getApiUrl(`${API_PATHS.TYPE_PREUVE}${id}/`));
              fetchData();
            } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing) {
        await api.put(getApiUrl(`${API_PATHS.TYPE_PREUVE}${currentId}/`), formData);
      } else {
        await api.post(getApiUrl(API_PATHS.TYPE_PREUVE), formData);
      }
  ...
  const renderHeader = () => (
    <View style={styles.tableHeader}>
      ...
      <View style={[styles.headerCell, { flex: 1 }]}><Text style={styles.headerText}>Type de Preuve</Text></View>
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <Text style={styles.title}>Types de Preuve</Text>
        </View>
      ...
      <Modal visible={modalVisible} transparent={true} animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.inputLabel}>Libellé du Type de Preuve</Text>
            <TextInput
...
const styles = StyleSheet.create({
  ...
  modalContent: { width: '90%', backgroundColor: '#fff', borderRadius: 15, padding: 15 },
...
export default TypePreuveManagementScreen;
