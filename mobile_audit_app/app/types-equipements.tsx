...
const TypesEquipementsManagementScreen = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get(getApiUrl(API_PATHS.TYPES_EQUIPEMENTS));
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              await api.delete(getApiUrl(`${API_PATHS.TYPES_EQUIPEMENTS}${id}/`));
              fetchData();
            } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing) {
        await api.put(getApiUrl(`${API_PATHS.TYPES_EQUIPEMENTS}${currentId}/`), formData);
      } else {
        await api.post(getApiUrl(API_PATHS.TYPES_EQUIPEMENTS), formData);
      }
  ...
  const renderHeader = () => (
    <View style={styles.tableHeader}>
      ...
      <View style={[styles.headerCell, { flex: 1 }]}><Text style={styles.headerText}>Type d'Équipement</Text></View>
...
export default TypesEquipementsManagementScreen;
