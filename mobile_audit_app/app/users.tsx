...
const UsersManagementScreen = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get(getApiUrl(API_PATHS.USERS));
  ...
  const handleEdit = (item) => {
    ...
    setFormData({ username: item.username, email: item.email || '', role: item.role || '' });
    setModalVisible(true);
  };
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              await api.delete(getApiUrl(`${API_PATHS.USERS}${id}/`));
              fetchData();
            } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing) {
        await api.put(getApiUrl(`${API_PATHS.USERS}${currentId}/`), formData);
      } else {
        await api.post(getApiUrl(API_PATHS.USERS), formData);
      }
  ...
  const renderHeader = () => (
    <View style={styles.tableHeader}>
      ...
      <View style={[styles.headerCell, { flex: 1 }]}><Text style={styles.headerText}>Utilisateur</Text></View>
      <View style={[styles.headerCell, { flex: 1.5 }]}><Text style={styles.headerText}>Email</Text></View>
      <View style={[styles.headerCell, { width: 70 }]}><Text style={styles.headerText}>Rôle</Text></View>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tableContainer}>
        ...
        {loading && !refreshing ? (
          ...
          <FlatList
            data={data.filter(item => item.username.toLowerCase().includes(search.toLowerCase()) || (item.email && item.email.toLowerCase().includes(search.toLowerCase())))}
